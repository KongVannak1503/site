...
import { Breadcrumb, Layout, Table, Space, Input, Button, Badge, Tooltip, Spin } from "antd";
...
import { deleteSkillApi, getSkillApi } from '../../../components/apis/SkillApi';
...
const Skills = () => {
    ...
    const canCreateUser = permissions['/skills']?.includes('create');
    const canUpdateUser = permissions['/skills']?.includes('update');
    const canDeleteUser = permissions['/skills']?.includes('delete');
    ...
    return (
        <Layout>
            <Breadcrumb
                items={[{ title: <span onClick={() => navigate("/dashboard")} style={{ cursor: "pointer", color: "#000" }}>Home</span> }, { title: <span style={{ cursor: "pointer", color: "#3a3a3a" }}>Skills</span> }]}
                style={{ margin: "16px 0" }}
            />
            <Content
                style={{
            ...
            >
                ...
                <CenterSlideModal isOpen={isModalOpen} onClose={closeModal} title={activeForm === "formUpdate" ? "Update Skill" : "Add New Skill"}>
                    {activeForm === "form" && <CreateSkill onUserCreated={handleAddCreated} onClose={closeModal} />}
                    {activeForm === "formUpdate" && updateUserId && (
                        <UpdateSkill userId={updateUserId} onUserUpdated={handleUpdate} onClose={closeModal} />
                    )}
                </CenterSlideModal>
            </Content>
        </Layout>
