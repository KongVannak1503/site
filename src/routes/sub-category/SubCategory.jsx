...
import { Breadcrumb, Layout, Table, Space, Input, Badge, Tooltip } from "antd";
...
import CreateCategory from './SubCreateCategory';
import UpdateCategory from './SubUpdateCategory';
import { NEW_USER } from '../../components/utils/Constants';
import { deleteSubCategoryApi, getSubCategoryApi } from '../../components/apis/SubCategoryApi';
...
const SubCategory = () => {
    ...
    const canCreateUser = permissions['/sub-category']?.includes('create');
    const canUpdateUser = permissions['/sub-category']?.includes('update');
    const canDeleteUser = permissions['/sub-category']?.includes('delete');
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "category",
            key: "category",
            render: (text, record) => <span>{record.categoryId?.name}</span>,
        },
        {
            title: 'Status',
            key: 'isActive',
    ...
    return (
        <Layout>
            <Breadcrumb
                items={[{ title: <span onClick={() => navigate("/dashboard")} style={{ cursor: "pointer", color: "#000" }}>Home</span> }, { title: <span style={{ cursor: "pointer", color: "#3a3a3a" }}>Sub Category</span> }]}
                style={{ margin: "16px 0" }}
            />
            <Content
                style={{
            ...
            >
                <div className="flex justify-between pb-5">
                    <h2 className={Styles.headTitle}>Sub Category</h2>
                ...
                />
                <CenterSlideModal isOpen={isModalOpen} onClose={closeModal} title={activeForm === "formUpdate" ? "Edit Sub Category" : "Add New Sub Category"}>
                    {activeForm === "form" && <CreateCategory onUserCreated={handleAddCreated} onClose={closeModal} />}
                    {activeForm === "formUpdate" && updateUserId && (
                        <UpdateCategory userId={updateUserId} onUserUpdated={handleUpdate} onClose={closeModal} />
                    )}
                </CenterSlideModal>
            </Content>
        </Layout>
